...
geometry_msgs::msg::Twist PurePursuitController::computeVelocity(const geometry_msgs::msg::PoseStamped &target) {
  ...
  cmd_vel.linear.y = linear_speed_;
  cmd_vel.linear.z = 0;

  // Adjust angular velocity based on curvature
  const tf2::Quaternion inverse_robot_q(
    robot_odom_->pose.pose.orientation.x,
    robot_odom_->pose.pose.orientation.y,
    robot_odom_->pose.pose.orientation.z,
    -1 * robot_odom_->pose.pose.orientation.w
  );
  ...
  double steering_angle = extractYaw(relative_q_quat);
